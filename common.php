...
if (isset($_SESSION['user_id'])) {
    ...
    if (!$CurUserID) {
        ...
        $_userInfo = $DB->row("SELECT ID,UserName,Salt,Password,UserRoleID,UserMail,UserIntro FROM " . PREFIX . "users WHERE UserName = :UserName", array(
            "UserName" => $_SESSION['user_id']
        ));
        $CurUserExpirationTime = 30 * 86400 + $TimeStamp;//默认保持30天登陆状态
        $NewUserSalt     = mt_rand(100000, 999999);
        $NewUserPassword = md5($_SESSION['user_id'] . $NewUserSalt);
        if (empty($_userInfo)) {
            $NewUserData = array(
                'ID' => null,
                'UserName' => $_SESSION['user_id'],
                ...
                'UserIntro' => '',
                'UserIM' => '',
                'UserRoleID' => isset($_SESSION['administrator']) ? 5 : 1,
                'UserAccountStatus' => 1,
            ...
            if (extension_loaded('gd')) {
                ...
                $Avatar = new MDAvtars(mb_substr($_SESSION['user_id'], 0, 1, "UTF-8"), 256);
            ...
            $CurUserID = $_userInfo['ID'];
            $NewUserSalt = $_userInfo['Salt'];
            $NewUserPassword = $_userInfo['Password'];
        }

        $CurUserCode = md5($NewUserPassword . $NewUserSalt . $CurUserExpirationTime . SALT);
        if (!$IsApp) {
            SetCookies(array(
                'UserID' => $CurUserID,
                'UserExpirationTime' => $CurUserExpirationTime,
                'UserCode' => $CurUserCode
            ), 30);
        }
        $CurUserName = $_userInfo['UserName'];
    }
    session_write_close();
} else {
    LogOut();
    // 没有登录跳到登录页面
    header('Location: /JudgeOnline/loginpage.php');
}
